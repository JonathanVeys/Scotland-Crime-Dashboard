...
from src.data_pipelines.preprocessing.spacial_processing import prepare_wards, calculate_overlap, load_and_prepare_shapefile, apply_disambiguation
from src.data_pipelines.DB.update_database import update_db

from src.data_pipelines.crime_rates_mapping import MANNUAL_WARD_NAME_EDITS, NAME_DISAMBIGUATION_2007, NAME_DISAMBIGUATION_2022


def crime_df_processing(df:pd.DataFrame, grouping_cols:List[str], crime_col:str):
    df = df.groupby(grouping_cols)[crime_col].sum().reset_index()
...
def main():
    ...
    for key in crime_data.keys():
        ...
        df = subset_columns(df, ['ward_name_2022', 'council_name', 'year', 'month', 'count'])
        df = df[~df['ward_name_2022'].isin(['.Other / Unknown', 'Other / Unknown'])]
        df['ward_name_2022'] = df['ward_name_2022'].map(
            lambda x: MANNUAL_WARD_NAME_EDITS.get(x,x)
        )
        df['ward_name_2022'] = df['ward_name_2022'].apply(normalise_text)
        df = df.groupby(['ward_name_2022', 'council_name', 'year', 'month'])['count'].sum().reset_index()

        df = df[~df['council_name'].isin(['Western Isles - Eilean Siar', 'North Ayrshire'])]
        df = df.merge(ward_code_2022_lookup, on='ward_name_2022', how='left').drop(['ward_name_2022', 'council_name'], axis=1)
        df = rename_column_names(df, {'ward_code_2022':'ward_code'})

        crime_data_list.append(df)
    
    crime_data = pd.concat(crime_data_list, ignore_index=True)
